...
my ($Edge,$Detail,$Len_p,$Len_t,$SubStr,$TrimMode,$Homopolymer,$APmode,$APdb,$Mismatch,$AddName,$Prefix,$Verbose,$Help);
my %opts;
GetOptions(\%opts,"trim_detail:s"=>\$Detail,"edge:i"=>\$Edge,"len_p:s"=>\$Len_p,"len_t:i"=>\$Len_t,"mis:i"=>\$Mismatch,
"sub:s"=>\$SubStr,"trim_mode:s"=>\$TrimMode,"trim_polymer:s"=>\$Homopolymer,"ap_mode"=>\$APmode,"ap_db:s"=>\$APdb, "addname:i"=>\$AddName,
"prefix:s"=>\$Prefix,
...
Option: --trim_detail <file>		filter_adapter detail file, include: "reads_id   reads_len   reads_start   reads_end   adapter_id"
		...
		--addname <int>		add trimmed info into seqname, 1-add else-no, default: 1
        --prefix <str>		set output prefix, support set output dir
...
foreach my $file(@ARGV)
{
	...
	my $outprefix=(defined $Prefix) ? $Prefix : $file; #(split /\//,$file)[-1];
	open (T,">$outprefix.trim.fastq") || die "Can't write to $file.trim.fastq\n";
	open (F,">$outprefix.filter.fastq") || die "Can't write to $file.filter.fastq\n";
	...
	while(<IN>)
	{
		if (/^([\@\>])(\S+)/)
		{
			...
			{
				my $db_title = '';
				for (my $k=0;$k<scalar(@{$Trim{$name}});$k+=4)
				{
					$db_title.=(defined $APdb && ${$Trim{$name}}[$k+3]=~/^\d+$/ && defined $APtitle[${$Trim{$name}}[$k+3]])?$APtitle[${$Trim{$name}}[$k+3]]." ":${$Trim{$name}}[$k+3]." ";
					if ( (${$Trim{$name}}[$k+0]-${$Trim{$name}}[$k+2]<=$Edge) || (defined $APmode && ${$Trim{$name}}[$k+0]-${$Trim{$name}}[$k+2]<${$Trim{$name}}[$k+1]-1) )
					{
						if (($TrimMode =~ /both/i)||($TrimMode eq "3"))
						{
							$Name .= " $db_title";
							$Name .= ("3\'_trimmed: ".(length($seq)-${$Trim{$name}}[$k+1])) if ($AddName==1);
							if (length($seq)-${$Trim{$name}}[$k+1]>0)
							{
								substr($seq,${$Trim{$name}}[$k+1]-$Left,length($seq)-${$Trim{$name}}[$k+1]," ");
								if ($type==0)
								{
									substr($qual,${$Trim{$name}}[$k+1]-$Left,length($qual)-${$Trim{$name}}[$k+1]," ");
								}
							}
							else
							{
					...
					elsif (${$Trim{$name}}[$k+1]<=$Edge || (defined $APmode && ${$Trim{$name}}[$k+0]-${$Trim{$name}}[$k+2]>${$Trim{$name}}[$k+1]-1))
					{
						if (($TrimMode =~ /both/i)||($TrimMode eq "5"))
						{
							my $kndex=$1 if ($Name=~/\:([ACGT]+)$/);
							my $seq_len=length($seq);
							if ($APmode)
							{
								$seq="";
							}
							elsif (${$Trim{$name}}[$k+2]<length($seq)+$Left)
							{
								substr($seq,0,${$Trim{$name}}[$k+2]-$Left," ");
							}
							if (length($seq)>0)
							{
								if (defined $kndex && $kndex ne "" && length($kndex)>3)
								{
									my @Index=split "",$kndex;
									for (my $k=0;$k<@Index-1;$k++)
									{
										my @tmp=@Index;
										$tmp[$k]="." if ($k>0);
										my $pattern=join "",@tmp;
										...
										{
											$seq="";
											${$Trim{$name}}[$k+2]=$seq_len;
											$Name .= "match Index after trimmed: $pattern" if ($AddName==1);;
											last;
										}
									}
								}
								if ($type==0)
								{
									if (${$Trim{$name}}[$k+2]<length($qual)+$Left)
									{
										substr($qual,0,${$Trim{$name}}[$k+2]-$Left," ");
									}
									else{
										$qual="";
									}
							...
							$Name .= " $db_title";
							$Name .= ("5\'_trimmed: ".(${$Trim{$name}}[$k+2]-${$Trim{$name}}[$k+1])) if ($AddName==1);;
							$trim_left++;
						}
						else
						{
			...
			for (my $i=0;$i<@{$APSeq{$seed}};$i++){
				...
				if ($len<length($apseq)){
					if ($rightpart eq substr($apseq,0,$len)){
						$$name .= " $spname";
						$$name .= "3\'_trimmed: $len" if ($AddName==1);;
						$$seq=$leftpart;
					}
				} else {
					if ($rightpart =~ /($apseq)/) {
						...
						$$name .= " $spname";
						$$name .= "3\'_trimmed: $trimlen" if ($AddName==1);;
						$$seq=$`;
					}
				}
			}
